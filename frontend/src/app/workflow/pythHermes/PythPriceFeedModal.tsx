...
const PRICE_FEEDS: PriceFeed[] = [
  ...
  },
  {
    ...
    name: 'Hedera',
    symbol: 'HBAR/USD',
    feedId: '0x9a7fb1b3950837a8d98269cbfdb5fcac260fa7f9b81a14f21d9fa3bc6e8d1a62'
  }
];
...
export default function PythPriceFeedModal({ isOpen, onClose, onConfirm }: PythPriceFeedModalProps) {
  ...
  const [selectedFeed, setSelectedFeed] = useState<PriceFeed>(PRICE_FEEDS[0]);
  ...
  const fetchLatestPrice = async () => {
    ...
    try {
      const response = await fetch(
        `${HERMES_URL}/v2/updates/price/latest?ids[]=${selectedFeed.feedId}`
      );
  ...
  const startStreaming = () => {
    ...
    const eventSource = new EventSource(
      `${HERMES_URL}/v2/updates/price/stream?ids[]=${selectedFeed.feedId}`
    );
  ...
  const handleFeedChange = (feed: PriceFeed) => {
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold" style={{ color: '#e0e8f0' }}>
              Pyth Price Feed - {selectedFeed.symbol}
            </h2>
          </div>
          <button
            onClick={onClose}
                  ...
                  <div className="relative options-dropdown">
                    ...
                    {showOptions && (
                      <div
                        ...
                        style={{
                          background: 'linear-gradient(135deg, rgba(40, 40, 48, 0.98), rgba(30, 30, 36, 0.98))',
                          border: '1px solid rgba(150, 180, 220, 0.3)',
                      ...
                      >
                        {PRICE_FEEDS.map((feed) => (
                          <button
                            ...
                            className="w-full px-4 py-2.5 text-left transition-all hover:bg-opacity-20 flex items-center justify-between"
                            style={{
                              background: selectedFeed.id === feed.id ? 'rgba(100, 150, 200, 0.2)' : 'transparent',
                              color: '#e0e8f0',
                          ...
                          >
                            <div>
                              ...
                              <div className="text-xs" style={{ color: '#8a9fb5' }}>{feed.symbol}</div>
                            </div>
                            {selectedFeed.id === feed.id && (
                              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" style={{ color: '#4ade80' }}>
